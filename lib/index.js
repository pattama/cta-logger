...
class LoggerClass extends Tool {
  constructor(dependencies, configuration) {
    super(dependencies, configuration);
    const result = common.validate(configuration.properties, {
    ...
    logger.author = function(id) {
      const conf = _.cloneDeep(configuration);
      conf.properties.author = id;
      return new LoggerClass(dependencies, conf);
    };
    return logger;
  }
}
...
function Logger(dependencies, configuration) {
  ...
  return new LoggerClass(_dependencies, _configuration);
}
